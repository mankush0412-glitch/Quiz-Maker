...
def parse_question_block(
    ...
    for i, line in enumerate(lines):
        ...
        if solution_start_idx is not None and i >= solution_start_idx:
            continue
        if question_lines and OPTION_PATTERN.match(line):
    ...
    for letter in option_letters:
        if letter in seen_letters:
            return None, (
                f"Duplicate option letter '{letter}' found. "
                f"Each option must have a unique letter (A, B, C, D...)"
            )
    ...
    solution_text = ((solution_text or "").strip()[:200]) or None
